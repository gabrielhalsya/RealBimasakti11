...
namespace GSM04500Service
{
    ...
    public class GSM04510GOAController : ControllerBase, IGSM04510GOA
    {
        ...
        [HttpPost]
        public R_ServiceDeleteResultDTO R_ServiceDelete(R_ServiceDeleteParameterDTO<GOADTO> poParameter)
        {
            throw new NotImplementedException();
        }
        [HttpPost]
        public R_ServiceGetRecordResultDTO<GOADTO> R_ServiceGetRecord(R_ServiceGetRecordParameterDTO<GOADTO> poParameter)
        {
            ...
            var loRtn = new R_ServiceGetRecordResultDTO<GOADTO>();
        ...
        [HttpPost]
        public R_ServiceSaveResultDTO<GOADTO> R_ServiceSave(R_ServiceSaveParameterDTO<GOADTO> poParameter)
        {
            string lcMethodName = nameof(R_ServiceSave);
            _loggerGSM04500.LogInfo(string.Format("START process method {0} on Controller", lcMethodName));

            R_Exception loException = new R_Exception();
            R_ServiceSaveResultDTO<GOADTO> loRtn = null;
            GSM04510GOACls loCls;

            try
            {
                loCls = new GSM04510GOACls();
                loRtn = new R_ServiceSaveResultDTO<GOADTO>();
        ...
        {
            string lcMethodName = nameof(GetJournalGrpGOAList);
            _loggerGSM04500.LogInfo(string.Format("START process method {0} on Controller", lcMethodName));

            var loEx = new R_Exception();
            GSM04510GOADBParameter loDbParameter;
            R_Exception loException = new R_Exception();
            IAsyncEnumerable<GOADTO> loRtn = null;
            List<GOADTO> loRtnTemp;
        ...
        private async IAsyncEnumerable<GOADTO> GET_JOURNAL_GROUP_GOA_LIST(List<GOADTO> poParameter)
        {
